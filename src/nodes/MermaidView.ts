...
export class MermaidView {
  ...
  private async render() {
    ...
    try {
      const { svg } = await mermaid.render('mermaid-' + Date.now(), this.text)
      this.renderHost.innerHTML = svg
      this.renderHost.style.display = ''
      this.cmHost.style.display = 'none'
      this.toggleBtn.textContent = 'Edit'
      this.mode = 'render'
    } catch {
      // Render failed — stay in edit mode
    }
  }
